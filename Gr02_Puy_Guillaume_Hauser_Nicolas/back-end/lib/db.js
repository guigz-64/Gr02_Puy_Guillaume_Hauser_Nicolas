...
module.exports = {
  ...
  messages: {
    create: async (message) => {
      if(!message.channelId) throw Error('Invalid channel')
      if(!message.author) throw Error('Invalid message')
      if(!message.content) throw Error('Invalid message')
      let creation = microtime.now()
      await db.put(`messages:${message.channelId}:${creation}`, JSON.stringify({
        author: message.author,
        content: message.content
      }))
      return merge(message, {channelId: message.channelId, creation: creation})
    },
